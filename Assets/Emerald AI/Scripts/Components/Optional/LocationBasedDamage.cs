...
namespace EmeraldAI
{
    public class LocationBasedDamage : MonoBehaviour
    {
        ...
        public Transform CustomTransformSource;
        public GetColliderSources GetColliderSource = GetColliderSources.ThisTransform;
        public enum GetColliderSources { ThisTransform, CustomTransfrom };
        [SerializeField]
           ...
           if (SetCollidersLayerAndTag) EmeraldDetection.LBDLayers |= (1 << LBDComponentsLayer);
            ...
            for (int i = 0; i < ColliderList.Count; i++)
            {
                if (ColliderList[i].ColliderObject.GetComponent<Rigidbody>() != null)
                {
                    ...
                    //Only set isKinematic to true if not using a custom transform source.
                    if (GetColliderSource == GetColliderSources.ThisTransform) ColliderRigidbody.isKinematic = true;
