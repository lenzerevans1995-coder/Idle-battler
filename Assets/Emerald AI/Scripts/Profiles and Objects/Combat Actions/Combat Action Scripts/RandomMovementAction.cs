...
namespace EmeraldAI
{
    ...
    public class RandomMovementAction : EmeraldAction
    {
        ...
        public MovementTypes MovementType = MovementTypes.BehindTarget;
        public enum MovementTypes { BehindTarget, BehindSelf };

        /// <summary>
        /// Continuously updates the EmeraldAction. This acts like an Update function that can run within this action using the information from the passed EmeraldComponent and its ActionClass.
        /// </summary>
        ...
        void GenerateRandomPositionWithinRadius(EmeraldSystem EmeraldComponent, ActionsClass ActionClass)
        {
            ...
            Vector3 Dir = EmeraldComponent.transform.position;

            if (MovementType == MovementTypes.BehindTarget)
            {
                Dir = (EmeraldComponent.CombatTarget.position - EmeraldComponent.transform.position).normalized;
            }

            if (MovementType == MovementTypes.BehindSelf)
            {
                Dir = -(EmeraldComponent.CombatTarget.position - EmeraldComponent.transform.position).normalized;
            }
            ...
            MoveCoroutine = EmeraldComponent.GetComponent<MonoBehaviour>().StartCoroutine(Moving(EmeraldComponent, ActionClass, GeneratedDestination));

            ActionClass.ActionCoroutine = MoveCoroutine;
        }

        IEnumerator Moving(EmeraldSystem EmeraldComponent, ActionsClass ActionClass, Vector3 Dest)
        {
            EmeraldComponent.m_NavMeshAgent.ResetPath();
            ...
            while (!CanCancel(EmeraldComponent) && EmeraldComponent.m_NavMeshAgent.enabled && !EmeraldComponent.AnimationComponent.IsDead && !Arrived)
            {
                bool ArrivedAtDest = HasArrived(EmeraldComponent.m_NavMeshAgent, Dest, 1f);

                if (EmeraldComponent.m_NavMeshAgent.remainingDistance >= 0.75f && !EmeraldComponent.m_NavMeshAgent.isOnOffMeshLink)
                {
                    EmeraldComponent.m_NavMeshAgent.stoppingDistance = 0.5f;
                ...
                else if (EmeraldComponent.m_NavMeshAgent.remainingDistance < 0.75f && !ArrivedAtDest)
                {
                    EmeraldComponent.m_NavMeshAgent.destination = Dest;

                    if (Vector3.Distance(EmeraldComponent.transform.position, EmeraldComponent.m_NavMeshAgent.destination) <= 0.75f)
                    {
                        Arrived = true;
                    }
                }
